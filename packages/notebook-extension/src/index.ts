...
namespace CommandIDs {
  ...
  export
  const createRegulus = 'notebook:create-regulus';
...
function activateNotebookHandler(app: JupyterLab, mainMenu: IMainMenu, palette: ICommandPalette, contentFactory: NotebookPanel.IContentFactory, editorServices: IEditorServices, restorer: ILayoutRestorer, launcher: ILauncher | null): INotebookTracker {
  ...
  app.contextMenu.addItem({command: CommandIDs.createRegulus, selector: '.jp-Notebook', rank: 3});
...
commands.addCommand(CommandIDs.createRegulus, {
    label: 'Create Regulus for Notebook',
    execute: args => {
      let current = getCurrent(args);
      console.log(current);
      if (!current) {
      ...
      console.log(options);
      return commands.execute('regulus:create', options);
    },
    isEnabled: hasWidget
  });
...
function createMenu(app: JupyterLab): Menu {
  ...
  menu.addItem({ command: CommandIDs.createRegulus });
  menu.addItem({ type: 'separator' });
